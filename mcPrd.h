...
template <class T> class European : public Product<T> {
  double myStrike;
  Time myExerciseDate;
...
public:
  ...
  European(const double strike, const Time exerciseDate,
           const Time settlementDate)
      : myStrike(strike), myExerciseDate(exerciseDate),
        mySettlementDate(settlementDate), myLabels(1) {
  ...
  void payoffs(
    ...
    payoffs.front() =
        max(spot - myStrike, 0.0) * sample.discounts.front() / sample.numeraire;
  }
};

template <class T> class UOC : public Product<T> {
...
public:
  ...
  UOC(const double strike, const double barrier, const Time maturity,
      const Time monitorFreq, const double smooth,
      const bool callPut = false) //	false = call, true = put
      : myCallPut(callPut), myStrike(strike), myBarrier(barrier),
        myMaturity(maturity), mySmooth(smooth), myLabels(2) {
    //  Timeline
    ...
    ost << " up and out " << myBarrier << " monitoring freq " << monitorFreq
        << " smooth " << mySmooth;
    myLabels[0] = ost.str();
  }
  ...
  void payoffs(
    ...
    //  untemplated
    const double smooth =
                     double(path.front().forwards.front().front() * mySmooth),
                 twoSmooth = 2 * smooth, barSmooth = myBarrier + smooth,
...
template <class T> class Europeans : public Product<T> {
  //	Timeline
...
public:
  ...
  const vector<Time> &maturities() const { return myMaturities; }

  const vector<vector<double>> &strikes() const { return myStrikes; }
  ...
  //  Timeline
  const vector<Time> &timeline() const override { return myMaturities; }
  ...
  void payoffs(
    ...
    for (size_t i = 0; i < numT; ++i) {
      transform(myStrikes[i].begin(), myStrikes[i].end(), payoffIt,
                [spot = path[i].forwards.front().front(),
                 num = path[i].numeraire](const double &k) {
...
template <class T> class ContingentBond : public Product<T> {
  Time myMaturity;
  double myCpn;
  double mySmooth;
...
public:
  ...
  ContingentBond(const Time maturity, const double cpn, const Time payFreq,
                 const double smooth)
  ...
  void payoffs(
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      const T s0 = start.forwards.front().front(),
              s1 = end.forwards.front().front();
      ...
      */
      ...
      payoffs.front() += digital                 //  contingency
