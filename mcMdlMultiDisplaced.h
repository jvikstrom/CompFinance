...
//  Multiple correlated assets, all following a displaced Black-Scholes (DLM)
//  dynamics, subject to discrete proportional dividends
//	DLMs are calibrated to short term ATM and skew, see
//https://www.researchgate.net/publication/335146739_Displaced_Lognormal_Models

//	On a dividend date: S+ = (1-div) * S-		--	by conv,
//dividend is applied end of day, that is, after everything else 	Prop divs don't
//affect BS implied surface
...
template <class T> class MultiDisplaced : public Model<T> {

  enum Dynamics { Lognormal = 0, Normal = 1, Surnormal = 2, Subnormal = 3 };
  ...
  //	Matrix (numDivDates, numAsets) of proportional dividends by date and
  //asset
  matrix<T> myDivs;
  ...
  //  forward factors for the dynamics, not to be confused with ffs for
  //  reconstruction
...
public:
  ...
  template <class U>
  MultiDisplaced(const vector<string> &assets, const U discRate,
                 const vector<U> &repoSpreads, const vector<U> &spots,
                 const vector<Time> &divDates, const matrix<U> &divs,
                 const vector<U> &atms, const vector<U> &skews,
                 const matrix<U> &correl, const U &lambda)
      : myNumAssets(assets.size()), myAssetNames(assets), myDiscRate(discRate),
        myDivDates(divDates), myDivs(divs), myLambda(lambda) {
    //	Copy data
    ...
    size_t numParams = 1 + 2 * numAssets + numAssets * divDates.size() +
                       numAssets + numAssets + numAssets * (numAssets - 1) / 2 +
                       1;
    ...
    for (size_t i = 1; i < numAssets; ++i) {
      for (size_t j = 0; j < i; ++j) {
        myParameterLabels[paramNum] =
            string("correl ") + myAssetNames[i] + " " + myAssetNames[j];
        ++paramNum;
      }
    }
...
public:
  ...
  const T rate() const { return myDiscRate; }

  const vector<T> &spots() const { return mySpots; }

  const vector<T> &repoSpreads() const { return myRepoSpreads; }

  const vector<Time> &divDates() const { return myDivDates; }

  const matrix<T> &divs() const { return myDivs; }

  const vector<T> &atms() const { return myAtms; }

  const vector<T> &skews() const { return mySkews; }

  const matrix<T> &correl() const { return myCorrel; }

  const T lambda() const { return myLambda; }

  const vector<Dynamics> &dynamics() const { return myDynamics; }

  const vector<T> &alphas() const { return myAlphas; }

  const vector<T> &betas() const { return myBetas; }

  //  Access to all the model parameters
  const vector<T *> &parameters() override { return myParameters; }
  ...
  void allocate(const vector<Time> &productTimeline,
    ...
    for (size_t j = 0; j < n; ++j)
      for (size_t k = 0; k < myNumAssets; ++k) {
...
private:
  ...
  T forwardFactor(const Time T1, const Time T2, const size_t asset) {
  ...
  void forwardFactors(const Time T1, const Time T2, T *fwdFacts) {
    const double dt = T2 - T1;
...
public:
  void init(const vector<Time> &productTimeline,
            const vector<SampleDef> &defline) override {
    transform(myRepoSpreads.begin(), myRepoSpreads.end(), myRepoRates.begin(),
              [=](const T &spread) { return myDiscRate - spread; });

    //	Find alphas and betas out of ATMs and skews, see
    //https://www.researchgate.net/publication/335146739_Displaced_Lognormal_Models
    ...
    transform(
        myCorrel.begin(), myCorrel.end(), myUsedCorrel.begin(),
        [&](const T &rawCorr) { return myLambda * (1.0 - rawCorr) + rawCorr; });

    //	Apply Choleski decomposition (see Numerical Recipes) to correlation
    //matrix 	If Wi are independent standard Gaussians, then: 	Bi = sum(CHOLij *
    //Wj) 	are correlated standard Gaussians with the correct correl
    ...
    for (size_t i = 0; i < m; ++i) {
      ...
      for (size_t j = 0; j < pDF; ++j) {
        myDiscounts[i][j] = exp(
            -myDiscRate * (defline[i].discountMats[j] - productTimeline[i]));
      }
...
private:
  ...
  template <class CONT>
  inline void fillScen(const size_t idx,     //  index on product timeline
    ...
    for (size_t a = 0; a < myNumAssets; ++a) {
      transform(fwdFacts[a].begin(), fwdFacts[a].end(),
                ...
                [spot = spots[a]](const T &ff) { return spot * ff; });
    }

    copy(myDiscounts[idx].begin(), myDiscounts[idx].end(),
         scen.discounts.begin());

    copy(myLibors[idx].begin(), myLibors[idx].end(), scen.libors.begin());
  }
  ...
  void generatePath(const vector<double> &gaussVec,
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      for (size_t a = 0; a < myNumAssets; ++a) {
        ...
        case Surnormal:
          spots[a] =
              (fwd + myAlphas[a]) * exp(myDrifts[i][a] + myStds[i][a] * cw) -
              myAlphas[a];
          break;
        case Subnormal:
        default:
