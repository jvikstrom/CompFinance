...
inline auto value(const Model<double> &model, const Product<double> &product,
  ...
  const auto resultMat =
      num.parallel ? mcParallelSimul(product, model, *rng, num.numPath)
  ...
  for (size_t i = 0; i < nPayoffs; ++i) {
    results.values[i] =
        accumulate(resultMat.begin(), resultMat.end(), 0.0,
...
//  AAD risk, one payoff
inline auto AADriskOne(const string &modelId, const string &productId,
                       const NumericalParam &num,
  ...
  //  Simulate
  const auto simulResults =
      num.parallel
          ? mcParallelSimulAAD(*product, *model, *rng, num.numPath,
                               [riskPayoffIdx](const vector<Number> &v) {
                                 return v[riskPayoffIdx];
                               })
  ...
  for (size_t i = 0; i < nPayoffs; ++i) {
    results.payoffValues[i] =
        accumulate(simulResults.payoffs.begin(), simulResults.payoffs.end(),
                   0.0,
                   [i](const double acc, const vector<double> &v) {
                     return acc + v[i];
                   }) /
  ...
  results.riskPayoffValue = accumulate(simulResults.aggregated.begin(),
...
inline auto AADriskAggregate(const string &modelId, const string &productId,
  ...
  if (!model || !product) {
    throw runtime_error(
        "AADriskAggregate() : Could not retrieve model and product");
  }
...
inline RiskReports AADriskMulti(const string &modelId, const string &productId,
                                const NumericalParam &num) {
  const Model<Number> *model = getModel<Number>(modelId);
  const Product<Number> *product = getProduct<Number>(productId);
...
inline RiskReports bumpRisk(const string &modelId, const string &productId,
                            const NumericalParam &num) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    *parameters[i] -= 1.e-08;
    ...
    for (size_t j = 0; j < m; ++j) {
      results.risks[i][j] = 1.0e+08 * (bumpRes.values[j] - baseRes.values[j]);
    }
  }
...
inline auto dupireAADRisk(
    ...
    //  product id
    const string &productId, const map<string, double> &notionals,
    //  numerical parameters
    const NumericalParam &num) {
  ...
  copy(next(simulResults.risks.begin()), simulResults.risks.end(),
       results.vega.begin());
...
inline auto dupireCalib(
    ...
    const double spot, const double vol, const double jmpIntens = 0.0,
    const double jmpAverage = 0.0, const double jmpStd = 0.0) {
  //  Create IVS
...
inline auto dupireSuperbucket(
    //  Model parameters that are not calibrated
    const double spot, const double maxDt,
    //  Product
    const string &productId, const map<string, double> &notionals,
    //  The local vol grid
    //  The spots to include
    ...
    //  The IVS we calibrate to
    //  Risk view
    const vector<double> &strikes, const vector<Time> &mats,
    //  Merton params
    const double vol, const double jmpIntens, const double jmpAverage,
    const double jmpStd,
    //  Numerical parameters
    const NumericalParam &num) {
  //  Results
  ...
  //  Calibrate the model
  auto params = dupireCalib(inclSpots, maxDs, inclTimes, maxDtVol, spot, vol,
  ...
  auto mdlDerivs = dupireAADRisk("superbucket", productId, notionals, num);
  ...
  auto nParams =
      dupireCalib(ivs, inclSpots, maxDs, inclTimes, maxDtVol, riskView);
  ...
  transform(riskView.begin(), riskView.end(), results.vega.begin(),
            [](const Number &n) { return n.adjoint(); });
...
//  Returns value, delta, strikes, maturities
//      and vega = derivatives to implied vols = superbucket
inline auto dupireSuperbucketBump(
    //  Model parameters that are not calibrated
    const double spot, const double maxDt,
    //  Product
    const string &productId, const map<string, double> &notionals,
  ...
  //  Base book value
  results.value =
      inner_product(vnots.begin(), vnots.end(), baseVals.values.begin(), 0.0);
  ...
  results.delta = (inner_product(vnots.begin(), vnots.end(),
                                 bumpedVals.values.begin(), 0.0) -
                   results.value) *
                  1.0e+08;
  ...
  for (size_t i = 0; i < n; ++i)
    for (size_t j = 0; j < m; ++j) {
      ...
      auto bumpedCalib =
          dupireCalib(ivs, inclSpots, maxDs, inclTimes, maxDtVol, riskView);
      //  Recreate model
      Dupire<double> bumpedModel(spot, bumpedCalib.spots, bumpedCalib.times,
                                 bumpedCalib.lVols, maxDt);
      //  Reprice
      auto bumpedVals = value(bumpedModel, *product, num);
      //  Pick results and differentiate
      results.vega[i][j] = (inner_product(vnots.begin(), vnots.end(),
                                          bumpedVals.values.begin(), 0.0) -
                            results.value) *
                           1.0e+05;
      //  Unbump
