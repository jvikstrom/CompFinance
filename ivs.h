...
//  Risk view
template <class T> class RiskView {
  bool myEmpty;
...
public:
  ...
  RiskView(const vector<double> &strikes, const vector<Time> &mats)
      : myEmpty(false), myStrikes(strikes), myMats(mats),
        mySpreads(strikes.size(), mats.size()) {
    for (auto &spr : mySpreads)
      spr = T(0.0);
  }

  //  Get spread
  T spread(const double strike, const Time mat) const {
    return myEmpty ? T(0.0)
...
public:
  ...
  //  Read access to spot
  double spot() const { return mySpot; }
  ...
  template <class T = double>
  T localVol(const double strike, const double mat,
             const RiskView<T> *risk = nullptr) const {
    //  Derivative to time
...
public:
  MertonIVS(const double spot, const double vol, const double intens,
            const double aveJmp, const double stdJmp)
      : IVS(spot), myVol(vol), myIntensity(intens), myAverageJmp(aveJmp),
        myJmpStd(stdJmp) {}

  double impliedVol(const double strike, const Time mat) const override {
    //  Merton's formula is defined in analytics.h
    const double call =
        merton(spot(), strike, myVol, mat, myIntensity, myAverageJmp, myJmpStd);
