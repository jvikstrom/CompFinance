...
template <class T> class Dupire : public Model<T> {
  //  Model parameters
...
public:
  ...
  template <class U>
  Dupire(const U spot, const vector<double> spots, const vector<Time> times,
         const matrix<U> vols, const Time maxDt = 0.25)
      : mySpot(spot), mySpots(spots), myLogSpots(mySpots.size()),
        myTimes(times), myVols(vols), myMaxDt(maxDt),
...
public:
  ...
  T spot() const { return mySpot; }

  const vector<double> &spots() const { return mySpots; }

  const vector<Time> &times() const { return myTimes; }

  const vector<T> &vols() const { return myVols; }
  ...
  void allocate(const vector<Time> &productTimeline,
    ...
    //  Do the fill
    myTimeline =
        fillData(productTimeline,               // Original (product) timeline
    ...
    transform(myTimeline.begin(), myTimeline.end(), myCommonSteps.begin(),
              [&](const Time t) {
                return binary_search(productTimeline.begin(),
                                     productTimeline.end(), t);
              });
  ...
  void init(const vector<Time> &productTimeline,
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      for (size_t j = 0; j < m; ++j) {
        myInterpVols[i][j] =
            sqrtdt * interp(myTimes.begin(), myTimes.end(), myVols[j],
                            myVols[j] + myTimes.size(), myTimeline[i]);
      }
    }
  }

  //  MC Dimension
  size_t simDim() const override { return myTimeline.size() - 1; }
...
public:
  ...
  void generatePath(const vector<double> &gaussVec,
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      T vol = interp(myLogSpots.begin(), myLogSpots.end(), myInterpVols[i],
                     myInterpVols[i] + m, logspot);
...
inline auto dupireCalib(
  ...
  for (size_t j = 0; j < n; ++j) {
    dupireCalibMaturity(ivs, results.times[j], results.spots.begin(),
                        results.spots.end(), lVolsT[j], riskView);
  }
