...
struct ToyNumber {
  ...
  ToyNumber &operator+=(const ToyNumber &rhs) {
    *this = *this + rhs;
    return *this;
  }
  ToyNumber &operator-=(const ToyNumber &rhs) {
  ...
  friend bool operator==(const ToyNumber &lhs, const ToyNumber &rhs) {
...
inline T blackScholes(
    //  input layer 0
    const T spot, const T rate, const T yield, const T vol, const T strike,
    const T mat) {
  /* layer 1 */ T df = exp(-rate * mat), fwd = spot * exp((rate - yield) * mat),
                  std = vol * sqrt(mat);
...
vector<double> calculateAdjoints(ToyNumber &result) {
  ...
  {
    if (tape[j].numArg > 0) {
      adjoints[tape[j].idx1] +=
          adjoints[j] * tape[j].der1; //  propagate first argument

      if (tape[j].numArg > 1) {
        adjoints[tape[j].idx2] +=
            adjoints[j] * tape[j].der2; //  propagate second argument
      }
    }
  }
...
void blackScholesDiff() {
  ToyNumber spot = 100, rate = 0.02, yield = 0.05, vol = 0.2, strike = 110,
            mat = 2; // initializes and records inputs
  auto result = blackScholes(spot, rate, yield, vol, strike,
                             mat); // evaluates and records operations
  ...
  //  show derivatives
  cout << "Derivative to spot (delta) = " << adjoints[spot.idx]
       << endl; //  0.309
  cout << "Derivative to rate (rho) = " << adjoints[rate.idx]
       << endl; //  51.772
  cout << "Derivative to dividend yield = " << adjoints[yield.idx]
       << endl; //  -61.846
  cout << "Derivative to volatility (vega) = " << adjoints[vol.idx]
       << endl; //  46.980
  cout << "Derivative to strike (-digital) = " << adjoints[strike.idx]
       << endl; //  -0.235
  cout << "Derivative to maturity (-theta) = " << adjoints[mat.idx]
       << endl; //  1.321
...
inline T toyDupireBarrierMc(
    ...
    const T S0,
    //  Local volatility
    const vector<T> spots, const vector<T> times, const matrix<T> vols,
    //  Product parameters
    const T maturity, const T strike, const T barrier,
    //  Number of paths and time steps
    const int Np, const int Nt,
    //  Smoothing
    const T epsilon,
  ...
  //  Initialize
  T result = 0;
  vector<double> gaussianIncrements(
      Nt); // double because the RNG is not templated (and doesn't need to be,
           // see chapter 12)
  ...
  for (int i = 0; i < Np; ++i) {
    ...
    for (size_t j = 0; j < Nt; ++j) {
      ...
      else if (spot < barrier - epsilon) { /* do nothing */
      }                                    //   definitely alive
      else                                 /* in between, interpolate */
        alive *= 1.0 - (spot - barrier + epsilon) / (2 * epsilon);
    }
    //  Payoff
    /* if (alive && spot > strike) result += spot - strike; */ if (spot >
                                                                   strike)
      result += alive * (spot - strike); // pay on surviving notional
  }                                      //  paths
...
void dupireRisksMiniBatch(
    const double S0, const vector<double> spots, const vector<double> times,
    const matrix<double> vols, const double maturity, const double strike,
    const double barrier, const int Np, const int Nt, const double epsilon,
    RNG &random,
    /* results: value and dV/dS, dV/d(local vols) */ double &price,
    double &delta, matrix<double> &vegas) {

  //	1. Initialize inputs, record on tape

  ToyNumber nS0(S0), nMaturity(maturity), nStrike(strike), nBarrier(barrier),
      nEpsilon(epsilon);
  vector<ToyNumber> nSpots(spots.size()), nTimes(times.size());
  matrix<ToyNumber> nVols(vols.rows(), vols.cols());

  for (int i = 0; i < spots.size(); ++i)
    nSpots[i] = ToyNumber(spots[i]);
  for (int i = 0; i < times.size(); ++i)
    nTimes[i] = ToyNumber(times[i]);
  for (int i = 0; i < vols.rows(); ++i)
    for (int j = 0; j < vols.cols(); ++j)
      nVols[i][j] = ToyNumber(vols[i][j]);

  //	2. Call instrumented evaluation code, which evaluates the barrier option
  //price and records all operations

  ToyNumber nPrice =
      toyDupireBarrierMc(nS0, nSpots, nTimes, nVols, nMaturity, nStrike,
                         nBarrier, Np, Nt, nEpsilon, random);
  ...
  for (int i = 0; i < vols.rows(); ++i)
    for (int j = 0; j < vols.cols(); ++j)
      vegas[i][j] = adjoints[nVols[i][j].idx];
}
...
void toyDupireBarrierMcRisks(
  ...
  while (pathsToGo > 0) {
    ...
    dupireRisksMiniBatch(S0, spots, times, vols, maturity, strike, barrier,
                         paths, Nt, epsilon, random, batchPrice, batchDelta,
                         batchVegas);
