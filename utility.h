...
inline CONT fillData(
    ...
    IT addBegin = nullptr, IT addEnd = nullptr) {
  //  Results
  CONT filled;

  //  Add points?
  CONT added;
  const std::size_t addPoints =
      addBegin && addEnd ? distance(addBegin, addEnd) : 0;

  if (addPoints > 0) {
    set_union(original.begin(), original.end(), addBegin, addEnd,
              back_inserter(added),
