...
public:
  ...
  void nextU(vector<double> &uVec) override {
    if (myAnti) {
      ...
      transform(myCachedUniforms.begin(), myCachedUniforms.end(), uVec.begin(),
      ...
      //	Generate and cache
      generate(myCachedUniforms.begin(), myCachedUniforms.end(),
               [this]() { return nextNumber(); });

      //	Copy
      copy(myCachedUniforms.begin(), myCachedUniforms.end(), uVec.begin());
  ...
  void nextG(vector<double> &gaussVec) override {
    if (myAnti) {
      ...
      transform(myCachedGaussians.begin(), myCachedGaussians.end(),
                gaussVec.begin(), [](const double n) { return -n; });

      //	Generate next
      myAnti = false;
    } else {
      //	Generate and cache
      generate(myCachedGaussians.begin(), myCachedGaussians.end(),
               [this]() { return invNormalCdf(nextNumber()); });

      //	Copy
      copy(myCachedGaussians.begin(), myCachedGaussians.end(),
           gaussVec.begin());
...
private:
  ...
  static void vPrd(const unsigned long long lhs[3][3],
                   ...
                   const unsigned long long mod, unsigned long long result[3]) {
    for (size_t j = 0; j < 3; j++) {
      unsigned long long s = 0;
      for (size_t l = 0; l < 3; l++) {
  ...
  void skipNumbers(const unsigned b) {
    ...
    static constexpr unsigned long long m1l =
        static_cast<unsigned long long>(m1);
    static constexpr unsigned long long m2l =
        static_cast<unsigned long long>(m2);

    unsigned long long Ab[3][3] = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}},
                       Bb[3][3] = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}},
                       Ai[3][3] =
                           {//  A0 = A
                            {
                                0, static_cast<unsigned long long>(a12),
                       ...
                       Bi[3][3] = {//  B0 = B
                                   {
                                       static_cast<unsigned long long>(a21), 0,
    ...
    //  Final result
    unsigned long long X0[3] = {static_cast<unsigned long long>(myXn),
                                static_cast<unsigned long long>(myXn1),
                                static_cast<unsigned long long>(myXn2)},
                       Y0[3] = {static_cast<unsigned long long>(myYn),
