...
Number::Number(const double val) : myValue(val) { createNode<0>(); }
...
void Number::putOnTape() { createNode<0>(); }
...
double &Number::value() { return myValue; }
double Number::value() const { return myValue; }
//  Single dimensional
double &Number::adjoint() { return myNode->adjoint(); }
double Number::adjoint() const { return myNode->adjoint(); }
//  Multi dimensional
double &Number::adjoint(const size_t n) { return myNode->adjoint(n); }
double Number::adjoint(const size_t n) const { return myNode->adjoint(n); }

//  Reset all adjoints on the tape
//		note we don't use this method
void Number::resetAdjoints() { tape->resetAdjoints(); }

//  Propagation

//  Propagate adjoints
//      from and to both INCLUSIVE
void Number::propagateAdjoints(Tape::iterator propagateFrom,
...
void Number::propagateToStart() { propagateAdjoints(tape->begin()); }
void Number::propagateToMark() { propagateAdjoints(tape->markIt()); }
...
void Number::propagateAdjointsMulti(Tape::iterator propagateFrom,
                                    Tape::iterator propagateTo) {
  auto it = propagateFrom;
  while (it != propagateTo) {
    it->propagateAll();
...
Number operator+(const double &lhs, const Number &rhs) { return rhs + lhs; }
...
//  Unary +/-
Number Number::operator-() const { return 0.0 - *this; }
