...
template <class T> class BlackScholes : public Model<T> {
  //  Model parameters
...
public:
  ...
  template <class U>
  BlackScholes(const U spot, const U vol, const bool spotMeasure = false,
               const U rate = U(0.0), const U div = U(0.0))
      : mySpot(spot), myVol(vol), myRate(rate), myDiv(div),
        mySpotMeasure(spotMeasure), myParameters(4), myParameterLabels(4) {
...
public:
  ...
  const T vol() const { return myVol; }

  const T rate() const { return myRate; }

  const T div() const { return myDiv; }
...
public:
  ...
  void generatePath(const vector<double> &gaussVec,
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      //  Black-Scholes
      spot = spot * exp(myDrifts[i] + myStds[i] * gaussVec[i]);
      //  Store on the path
      fillScen(idx, spot, path[idx], (*myDefline)[idx]);
      ++idx;
    }
